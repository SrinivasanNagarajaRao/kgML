...
import net.kugou.algorithms.dbscan.spatial._
import net.kugou.algorithms.dbscan.spatial.rdd.{PartitioningSettings, PointsInAdjacentBoxesRDD, PointsPartitionedByBoxesRDD}
import net.kugou.algorithms.dbscan.util.debug.DebugHelper
...
/** Implementation of the DBSCAN algorithm which is capable of parallel processing of the input data.
  ...
  *
  * @param settings Parameters of the algorithm. See [[net.kugou.algorithms.dbscan.DbscanSettings]]
  *                 for details
  * @param partitioningSettings Partitioning settings. See [[net.kugou.algorithms.dbscan.spatial.rdd.PartitioningSettings]]
  *                             for details
  */
class DistributedDbscan (
   ...
   *
   * @param data A data set to be clustered. See [[net.kugou.algorithms.dbscan.RawDataSet]] for details
   * @return A [[net.kugou.algorithms.dbscan.DbscanModel]] object which represents clustering results
   */
  ...
  /** Finds clusters within one partition
    ...
    *                    when this method is called, [[net.kugou.algorithms.dbscan.spatial.Point]] objects in the collection
    *                    already contain all necessary metadata, including number of their neighbors
    *
    * @param boundingBox A [[net.kugou.algorithms.dbscan.spatial.Box]] object which represents bounds of the whole dataset
  ...
  private def generateMappings (pointsCloseToBoxBounds: RDD[Point], boxes: Iterable[Box])
    :(mutable.HashSet[(mutable.HashSet[ClusterId], ClusterId)], Map [PointId, ClusterId])  = {
    ...
    val pairwiseMappings: RDD[(ClusterId, ClusterId)] = pointsInAdjacentBoxes.mapPartitionsWithIndex {
      (_, it) => {
        ...
        val pairs = mutable.HashSet[(ClusterId, ClusterId)] ()
    ...
    val mappings = mutable.HashSet[mutable.HashSet[ClusterId]] ()
    val processedPairs = mutable.HashSet[(ClusterId, ClusterId)] ()
  ...
  /** Detects clusters in different partitions which should be merged together and generates mappings
    ...
    */
  private [dbscan] def generateMappings (localData: Seq[Point]): (mutable.HashSet[(mutable.HashSet[ClusterId], ClusterId)],
    ...
    println(s"Number of points: $numPoints ; iterations: $innerLoopIterations")
  ...
  private def processPairOfClosePoints (pt1: Point,
                                        ...
                                        c1: ClusterId,
                                        c2: ClusterId,
                                        processedPairs: mutable.HashSet[(ClusterId, ClusterId)],
                                        mappings: mutable.HashSet[mutable.HashSet[ClusterId]]): Unit = {
    ...
    if (!processedPairs.contains(pair)) {
      ...
      (m1, m2) match {
        ...
        case (None, y: Some[mutable.HashSet[ClusterId]]) => y.get += c1
        case (x: Some[mutable.HashSet[ClusterId]], None) => x.get += c2
        case (x: Some[mutable.HashSet[ClusterId]], y: Some[mutable.HashSet[ClusterId]]) =>
          if (x != y) {
  ...
  private [dbscan] def reassignClusterId (
                                           pt: Point,
                                           mappings: mutable.HashSet[(mutable.HashSet[ClusterId], ClusterId)],
    ...
    mapping match {
      case m: Some[(mutable.HashSet[ClusterId], ClusterId)] => newClusterId = m.get._2
      case _ =>
    }
