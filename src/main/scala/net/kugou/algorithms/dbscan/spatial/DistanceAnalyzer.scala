...
import net.kugou.algorithms.dbscan.{DbscanSettings, PairOfAdjacentBoxIds, PointCoordinates}
import net.kugou.algorithms.dbscan.spatial.rdd.{BoxPartitioner, PartitioningSettings, PointsInAdjacentBoxesRDD, PointsPartitionedByBoxesRDD}
...
private [dbscan] class DistanceAnalyzer (
  ...
  def countNeighborsForEachPoint ( data: PointsPartitionedByBoxesRDD )
    ...
    val sortedData = data.mapPartitions (sortPartition, preservesPartitioning = true)

    val pointsWithCounts = sortedData.zipPartitions(partitionedAndSortedCounts, preservesPartitioning = true) {
      (it1, it2) => {
        it1.zip (it2).map {
          x => {
