package net.kugou.applications.sexPredict.train
...
/**
  *
  * Created by yhao on 2017/12/26 18:04.
  */
object SexClassTrain extends Serializable {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //加载数据SQL
    var sql: String = s"select userid, cast($label as int) as sex, nickname, useralias, app_list, play_songid_list, top_singer, '$date' as dt, pt " +
    ...
    train(nonZeroData, "sex", "features", "predictions", iterNum, modelPath)
  ...
  def loadData(spark: SparkSession, sql: String, numPart: Int = 2000): DataFrame = {
    ...
    val result: DataFrame = spark.sql(sql).rdd.map { row =>
      ...
      val sex: Double = if(Option(row.get(1)).isEmpty) 2 else row.getInt(1).toDouble
      ...
      val dt: String = if(Option(row.getString(7)).isEmpty) "" else row.getString(7)
      val pt: String = if(Option(row.getString(8)).isEmpty) "" else row.getString(8)

      (userid, sex, useralias, applist, songidlist, singerlist, dt, pt)
    }.toDF("userid", "sex", "useralias", "applist", "songidlist", "singerlist", "dt", "pt")
