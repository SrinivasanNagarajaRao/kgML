...
/**
  * Author: yhao
  * Time: 2018/06/25 15:18
  * Desc: 根据已有高校中心点判断用户经纬度是否在高校内
  *
  * 高校中心点：
  * mllab.stu_jwd 手工收集
  * mllab.stu_jwd2 根据统计聚类
  * mllab.stu_jwd3 dbscan聚类
  *
  */
object DBSCANCollegePredictDemo extends Serializable {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val centersFilePath: String = "hdfs://kgdc/user/haoyan/data/centers.dat"
    ...
    val centerMap = centerData.flatMap{line =>
      ...
      if (tokens.length > 4) {
        ...
        if (count > 150 && (dist/count) < 0.8) Some((index, List((clusterId, longitude, latitude, dist, count)))) else None
      } else None
    ...
    spark.sql("use mllab")
    spark.sql("insert into mllab.stu_user_jwd_dbscan_detail_d partition(dt, pt) " +
