...
import org.apache.spark.ml.feature.{CountVectorizerModel, IDF, IDFModel, VectorAssembler}
...
/**
  ...
  *
  */
object StuClassPredict extends Serializable {
  private val logger: Logger = Logger.getLogger(this.getClass)

  def main(args: Array[String]): Unit = {
    Logger.getLogger("org").setLevel(Level.WARN)

    val algorithmType: String = "nb"
...
//    val idfModel: IDFModel = IDFModel.load(modelPath + "idfModel")
//    val idfData: DataFrame = idfModel.transform(vecData)
  ...
  def loadTestData(spark: SparkSession): DataFrame = {
    val surveySQL: String = "select t1.userid, case when t1.profession like '%大学生%' then 1 else 0 end label " +
      "from temp.survey_userid_C_0614_true t1 left outer join temp.survey_userid_C_0611_ture_bak t2 on t1.userid=t2.userid " +
      "where t2.userid is null"
    val userSQL: String = "select userid from temp.sljin_diaoyan_msgid where type='all_rand_1'"
    ...
    println(s"Load survey data sql: \n\t$surveySQL")
    println(s"Load user data sql: \n\t$userSQL")
    println(s"Load app data sql: \n\t$appSQL")
    println(s"Load city data sql: \n\t$citySQL")
    println(s"Load jwd data sql: \n\t$jwdSQL")

    val surveyData: DataFrame = spark.sql(surveySQL)
    val userData: DataFrame = spark.sql(userSQL).join(surveyData, "userid")
